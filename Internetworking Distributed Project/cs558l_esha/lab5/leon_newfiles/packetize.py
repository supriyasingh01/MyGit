...
def packets(filename, buf_size, data_size,num_of_servers, index):
	...
	stepsize = int(math.ceil(fsize / num_of_servers)) 	
	...
	num_of_packets = round(fsize/data_size) + 1	

	file.seek(forward)
	

	#while(len(data) >= data_size):#create each packet along with relevant information
	for i in range(0, int(math.ceil(num_of_packets/2))):
		...
		#print "Values of what Im sending", data
