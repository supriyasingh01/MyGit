...
u16 checksum_calc(int count, unsigned char *addr)
{
	   register long sum = 0;
       u16 checksum=0;
...
{
    return (((value & 0xFF) << 8) & 0xFF00) + ((value >> 8) & 0xFF);
}
...
void got_packet( u_char *packet,char *dev,int len,pcap_t * descr)
{
	...
	struct sniff_ip temp_ip = *ip;
	
	ip_src = ip -> ip_src;
	ip_dst = ip -> ip_dst;
	bpf_u_int32 dst_input = ip_dst.s_addr;

    printf("Old checksum  %u TTL %u\n",temp_ip.ip_sum, (u_int)temp_ip.ip_ttl);
  //Set Checksum to 0
    temp_ip.ip_sum = 0;
    //decrement TTL
    temp_ip.ip_ttl = temp_ip.ip_ttl - 1;
    
    /*
    if(temp_ip.ip_ttl == 0 )
    {
    	// send ICMP reject message TYPE: TIME EXCEEDED type 11
    }  
    */
    //calculate new Checksum
    //Add VHL+TOS
    newsum = ((temp_ip.ip_vhl << 8) & 0xFF00) + temp_ip.ip_tos;
    //Add reversed LEN
    newsum += reverse(temp_ip.ip_len);
    //Add reversed ID
    newsum += reverse(temp_ip.ip_id);
    //Add reversed Offset
    newsum += reverse(temp_ip.ip_off);
    //Add TTL+Protocol
    newsum += ((temp_ip.ip_ttl << 8) & 0xFF00) + temp_ip.ip_p;
    //Add reversed Src Address (MSB)
    newsum += reverse((temp_ip.ip_src.s_addr >> 16) & 0xFFFF);
    //Add reversed Src Address (LSB)
    newsum += reverse(temp_ip.ip_src.s_addr & 0xFFFF);
    //Add reversed Dst Address (MSB)
    newsum += reverse((temp_ip.ip_dst.s_addr >> 16) & 0xFFFF);
    //Add reversed Dst Address (LSB)
    newsum += reverse(temp_ip.ip_dst.s_addr & 0xFFFF);
    //Complement the newsum and reverse it to add to packet
    newsum = reverse(~(newsum & 0xFFFF));
    
    //newsum = checksum_calc(size_ip, (unsigned char *)packet + SIZE_ETHERNET);
    printf("IP Source: %s\n",(char *)inet_ntoa(ip_src));
    
    printf("New checksum  %u TTL %u\n",newsum, (u_int)temp_ip.ip_ttl);
    //memcpy(packet + SIZE_ETHERNET, &temp_ip, sizeof(struct sniff_ip));

	memcpy(packet + SIZE_ETHERNET + 8, (u8 *)&temp_ip.ip_ttl, 1);						// TTL
	memcpy(packet + SIZE_ETHERNET + 10, (u16 *)&newsum, 2);								// Checksum
