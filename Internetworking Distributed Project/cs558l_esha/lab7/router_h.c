...
void got_packet( u_char *packet,char *dev,int len,pcap_t * descr)
{
    ...
    //Add VHL+TOS
    newsum = ((ip->ip_vhl << 8) & 0xFF00) + ip->ip_tos;
    //Add reversed LEN
    newsum += reverse(ip->ip_len);
    //Add reversed ID
    newsum += reverse(ip->ip_id);
    //Add reversed Offset
    newsum += reverse(ip->ip_off);
    //Add TTL+Protocol
    newsum += ((ip->ip_ttl << 8) & 0xFF00) + ip->ip_p;
    //Add reversed Src Address (MSB)
    newsum += reverse((ip->ip_src.s_addr >> 16) & 0xFFFF);
    //Add reversed Src Address (LSB)
    newsum += reverse(ip->ip_src.s_addr & 0xFFFF);
    //Add reversed Dst Address (MSB)
    newsum += reverse((ip->ip_dst.s_addr >> 16) & 0xFFFF);
    //Add reversed Dst Address (LSB)
    newsum += reverse(ip->ip_dst.s_addr & 0xFFFF);
    //Complement the newsum and reverse it to add to packet
    ip->ip_sum = reverse(~(newsum & 0xFFFF));
